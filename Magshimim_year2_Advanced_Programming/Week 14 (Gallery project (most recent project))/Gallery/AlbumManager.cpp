...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Avarage Tags per Album: " << m_dataAccess.averageTagsPerAlbumOfUser(user) << std::endl << // there was a spelling error
		"  + Count of users Albums: " << m_dataAccess.countAlbumsOwnedOfUser(user) << std::endl;
}
