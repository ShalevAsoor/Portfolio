...
void MemoryAccess::deleteUserAlbums(const User& user)
{
	if (doesUserExists(user.getId()))
	{
		const std::list<Album>& userAlbums = getAlbumsOfUser(user);
		for (auto i = userAlbums.begin(); i != userAlbums.end(); ++i)
		{
			m_albums.remove(*i);
		}
	}
